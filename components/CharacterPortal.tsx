...
export const CharacterPortal: React.FC<CharacterPortalProps> = ({
  ...
  return (
    <div className="relative flex flex-col items-center justify-center w-full max-w-2xl mx-auto my-8">
      {/* Outer Glow Ring */}
      <div
        className={`absolute inset-0 rounded-full blur-3xl transition-all duration-700 ${isSpeaking ? 'bg-amber-500/30 scale-110' :
            isThinking || isGeneratingAvatar ? 'bg-blue-500/20 scale-105 animate-pulse' : 'bg-transparent'
          }`}
      ></div>

      {/* Main Portal Container */}
      <div className="relative z-10 w-48 h-48 md:w-80 md:h-80 rounded-full border-4 border-amber-500/20 overflow-hidden glass-panel flex items-center justify-center shadow-2xl transition-all duration-500 bg-black/40">
